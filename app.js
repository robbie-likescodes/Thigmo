...
function drawTile(pos, movable, selected){
  ...
  ctx.lineWidth = 1;
  ctx.stroke();
  ctx.strokeStyle = 'rgba(42,36,28,0.36)';
  ctx.lineWidth = 1.6;
  ctx.stroke();
}
...
function fitCameraToBoard(){
  ...
  const targetH = canvas.height * 0.48;
...
function draw(){
  ...
  for(const {tid,pos} of orderedTiles){
    const movable = state.legalMoves.some((m)=>m.tid===tid);
    drawTile(pos, (state.phase==='selectTile' || state.phase==='selectDest') && movable, state.hoverTileId===tid || state.selectedTileId===tid);
  }
  drawVineConnections();

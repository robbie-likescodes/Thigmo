...
function playableNeutralNodes(){
  const nodes = [];
  for (const [, pos] of state.tiles) {
    const stackHeight = (state.stacks.get(key(pos.x, pos.y)) || []).length;
    nodes.push({ x: pos.x, y: pos.y, z: stackHeight });
  }
  return nodes;
}

function nearestPlayableNeutral(mx, my){
  if (state.phase !== 'place') return null;
  let best = null;
  let bestScore = 1e9;
  for (const node of playableNeutralNodes()) {
    const c = worldToScreen(node.x, node.y, node.z * FLOWER_VERTICAL_SPACING);
    const d = Math.hypot(mx - c.sx, my - c.sy);
    const score = d / Math.max(12, projectedTileRadius(node.x, node.y) * 0.42);
    if (score < bestScore) {
      bestScore = score;
      best = node;
    }
...
function draw(){
  ...
  const mountainLayers = [
    {
      color: 'rgba(137, 178, 182, 0.44)',
      baseOffset: -18,
      points: [
        [0.00, 30], [0.06, 8], [0.12, 36], [0.18, 4], [0.24, 33], [0.30, -2],
        [0.36, 28], [0.43, -10], [0.50, 34], [0.57, 2], [0.64, 30], [0.71, -12],
        [0.78, 29], [0.86, 0], [0.93, 24], [1.00, 10],
      ],
    },
    {
      ...
      baseOffset: -8,
      points: [
        [0.00, 26], [0.07, 6], [0.14, 31], [0.21, 3], [0.28, 28], [0.35, -5],
        [0.42, 24], [0.49, -14], [0.56, 30], [0.63, 0], [0.70, 26], [0.78, -15],
        [0.86, 25], [0.93, -2], [1.00, 14],
      ],
    },
    {
      color: 'rgba(94, 136, 145, 0.46)',
      baseOffset: 2,
      points: [
        [0.00, 24], [0.08, 4], [0.16, 30], [0.24, 1], [0.32, 25], [0.40, -8],
        [0.48, 28], [0.56, -1], [0.64, 23], [0.72, -13], [0.80, 27], [0.88, 2],
        [0.94, 20], [1.00, 10],
      ],
    },
    {
      color: 'rgba(78, 118, 130, 0.40)',
      baseOffset: 10,
      points: [
        [0.00, 18], [0.10, -2], [0.18, 22], [0.27, -6], [0.36, 19], [0.45, -11],
        [0.53, 23], [0.61, -4], [0.70, 20], [0.78, -12], [0.86, 18], [0.94, -1], [1.00, 8],
      ],
    },
  ];
  ...
  if (ui.showNeutralsToggle?.checked || state.hoverNeutralPlacement) drawNeutralAssist();
